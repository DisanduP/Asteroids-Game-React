  ...
  class Ship {
    ...
    update(keys, shield) {
      ...
      if (keys["ArrowDown"] || keys["s"] || keys["S"]) {
        this.velocity.x -= Math.cos(this.angle) * this.thrust;
        this.velocity.y -= Math.sin(this.angle) * this.thrust;
        // Optional: different backward thrust particle color
        this.addThrustParticle(true);
      }
    ...
    addThrustParticle(isBackward = false) {
      if (Math.random() < 0.6) {
        const dir = isBackward ? -1 : 1;
        const backX = this.x - Math.cos(this.angle) * 12 * dir;
        const backY = this.y - Math.sin(this.angle) * 12 * dir;
        this.thrustParticles.push({
          x: backX + (Math.random() - 0.5) * 4,
          y: backY + (Math.random() - 0.5) * 4,
          vx:
            -Math.cos(this.angle) * (2 + Math.random() * 2) * dir +
            (Math.random() - 0.5),
          vy:
            -Math.sin(this.angle) * (2 + Math.random() * 2) * dir +
            (Math.random() - 0.5),
    ...
    draw(ctx, shield) {
      ...
      this.thrustParticles.forEach((p) => {
        ...
        ctx.fillStyle = p.backward
          ? `rgba(0,200,255,${alpha})`
          : `rgba(255, ${100 + p.life * 3}, 0, ${alpha})`;
    ...
    for (let i = bulletsRef.current.length - 1; i >= 0; i--) {
      for (let j = asteroidsRef.current.length - 1; j >= 0; j--) {
        if (asteroidsRef.current[j].checkCollision(bulletsRef.current[i], 3)) {
          // Save asteroid size before splicing!
          const asteroidSize = asteroidsRef.current[j].size;

          // Create explosion particles
          for (let k = 0; k < 6; k++)
            particlesRef.current.push(
              new Particle(asteroidsRef.current[j].x, asteroidsRef.current[j].y)
            );

          // Add score
          setHUD((prev) => ({
            ...prev,
            score: prev.score + (4 - asteroidSize) * 20,
          }));
          ...
          if (asteroidSize > 1) {
            for (let k = 0; k < 2; k++) {
              ...
              asteroidsRef.current.push(
                new Asteroid(newX, newY, asteroidSize - 1)
              );
            }
          }
  ...
  return (
    <div className="asteroids-root">
      ...
      <div className="controls">
        <div>↑ Thrust | ↓ Reverse | ← → Rotate | SPACE: Shoot</div>
